...
class BookFinder {
  ...
  async searchLibGen(query, timeout) {
    ...
    try {
      ...
      const { stdout, stderr } = await execFileAsync(
        'curl',
        ['-s', '-L', '-m', String(timeout / 1000), '--max-filesize', '10485760', url],
  ...
  async downloadFromLibGen(pageUrl, outputPath) {
    try {
      ...
      const { stdout: pageHtml } = await execFileAsync(
        'curl',
        ['-s', '-L', '-m', '30', '--max-filesize', '5242880', pageUrl],
  ...
  async downloadFile(url, outputPath) {
    ...
    try {
      ...
      await execFileAsync(
        'curl',
        ['-L', '-o', outputPath, '-m', '600', '--max-filesize', String(this.maxFileSize), url],
